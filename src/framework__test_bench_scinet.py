    ...
    *******************************************************************************************************************
    ...
    def __give_one_test_to_model(self, test_sample, model, should_print, dataset):
        ...
        mse_here, precision, recall, f1, mase, mape = self.__get_mse_precision_recall_f1_mase_and_mape(
            y_true=out_should_be[how_much_to_give:], y_pred=returned_ts_as_np_array[-how_much_to_predict:],
    ...
    def __test_model(self, test, model):
        ...
        for i, test_sample in enumerate(test):
            mse_here, precision, recall, f1, mase, mape = self.__give_one_test_to_model(
                test_sample=test_sample, model=model, should_print=False,dataset=test 
            )
            total_mse += mse_here
            ...
            total_mase += mase
            total_mape += mape
            precisions_list.append((precision, i))
            
        ##################################
        precisions_list.sort(key=lambda x: x[0])
        best_idx_list = [x[1] for x in precisions_list[:15]]
        for i, test_sample in enumerate(test):
            mse_here, precision, recall, f1, mase, mape = self.__give_one_test_to_model(
                test_sample=test_sample, model=model, should_print=(i in best_idx_list),dataset=test
            )
        ##################################
